...
- 📊 **Статистика**: по дням, за 7 дней с графиком, топ-3 за неделю/месяц
...
**Mini App (основной путь):** menu button «📝 Записать» в боте (или кнопка «⚡ Открыть Mini App» в `/quick`) → сетка активностей → тап по плитке → пресеты часов (0.5–3.5 шагом 0.5, дальше целыми до 15) + ✏️ custom + ✕ сброс. Переключатель Сегодня/Вчера/📅. В футере — итог дня и индикатор синка.
...
│   │   └── server.py        # FastAPI: /api/hobbies, /api/day, /api/entry, статика
│   ├── bot/
