...
- ⭐ **Гибкая система времени**: Оценивайте время от 0.5 до 8+ часов с поддержкой custom значений
...
- ⚙️ **Настройки**: Удобное меню для управления напоминаниями, алиасами и статистикой
- ✏️ **Custom ввод**: Возможность ввода произвольных значений времени с поддержкой десятичных чисел
...
### Готовые значения (кнопки)
- 🌟 (0.5) — Полчаса (30 минут)
- ⭐ (1-8) — От 1 до 8 часов времени
- ❌ (0) — Не было активности

### Custom ввод (✏️)
- Любые десятичные значения: 1.5, 2.75, 12, 15.5
- Поддержка форматов: "12,5" и "12.5"
- Диапазон: от 0 до 24 часов
- Автоматический возврат к главному меню
...
├── data/                   # Данные (монтируется в Docker, не в git)
...
│   └── stars.txt.example   # Пример конфигурации звезд
├── main.py                 # Точка входа
...
- **Быстрые кнопки статистики**: Кнопки "📊 Сегодня" и "📊 Вчера" показывают статистику сразу
- **Статистика**: Детальная статистика с общими баллами, доступная через настройки
...
- **Меню настроек**: Организованный доступ к статистике, напоминаниям и алиасам
- **Custom значения**: Ввод произвольных значений времени через текст с валидацией
- **Редактирование на сервере**: Файлы в `data/` доступны для прямого редактирования

## ⚙️ Настройка значений звезд

Вы можете настроить доступные значения времени (звезд) через файл `data/stars.txt`:

1. **При первом запуске** создается `data/stars.txt` с базовыми значениями
2. **Для кастомизации** скопируйте `data/stars.txt.example` в `data/stars.txt`
3. **Отредактируйте файл**, добавив нужные значения (одно на строку)
